...
# NOTE:move this file to the root
...
def build_multiple_emails_personalized():
    # Note that the domain for all email addresses must match
...
def build_kitchen_sink():
    ...
    # The values below are comment are global to entire message
...
## this will only send an email to if set SandBox Mode to False
# send_kitchen_sink()